    ...
    <div class="flex items-start gap-3">
      ...
      <div class="flex-1 min-w-0">
        <div class="flex items-center gap-1.5 mb-0.5">
          ...
          <!-- Free badge -->
          <span v-if="tool.free" class="text-xs bg-green-50 dark:bg-green-900/30 text-green-600 dark:text-green-400 px-1.5 py-0.5 rounded font-medium flex-shrink-0">{{ $t('tool.free') }}</span>
        </div>
        <p class="text-xs text-gray-500 dark:text-gray-400 leading-relaxed line-clamp-2">{{ displayDesc }}</p>

        <!-- Tags -->
        <div v-if="displayTags.length" class="flex gap-1 mt-2 flex-wrap">
          <span
